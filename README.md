...
在计算分数时，治理票占比会乘以治理票占比倍数，用于调整治理票在分数计算中的权重。当治理票占比倍数为 0 时，治理票不参与激励计算，激励仅由 LP 代币占比和区块比例决定。
...
- 区块比例 = 对于加入轮次，从当轮最后一次加入区块到加入轮次结束的区块数 / 阶段总区块数；对于非加入轮次，则为 1
- 退出轮次不计算激励
